...
public class TaskManager implements ITaskManager {
    ...
    public ITask getTaskByIdForUser(int taskId, IUser user) {
        ...
        for (TaskDAO taskDao : userTasks){
            if (taskDao.getId() == taskId){
                return mappers.getTaskMapper().mapFromDAODeep(taskDao);
            }
        }
        return null;
    }
    ...
    public void deleteTaskByIdForUser(int taskId, IUser user) {
        ...
        for (TaskDAO dao : userTasks){
            if (dao.getId() == taskId){
                foundTaskIndex = userTasks.indexOf(dao);
            }
        }
        userTasks.remove(foundTaskIndex);
