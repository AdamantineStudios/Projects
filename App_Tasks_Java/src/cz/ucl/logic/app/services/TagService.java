...
public class TagService implements ITagService {
    ...
    public void updateTag(int id, String title, Color color) {
        ITag foundTag = manager.getTagByIdForUser(id,userService.getUserLoggedIn());
        ITag updatedTag = new Tag(userService.getUserLoggedIn(), id, title, color, foundTag.getCreatedAt(), LocalDateTime.now());
        for (ITask task : foundTag.getTasks()){
            updatedTag.addTask(task);
        }
